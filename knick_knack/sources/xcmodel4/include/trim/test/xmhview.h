...
#define MAXVER     102400 /* max. number of vertices */
#define MAXSEG     409600 /* max. number of segments */
#define MAXFACE    102400 /* max. number of facets */
...
#define	GET_FACE4(fd,v0,v1,v2,v3) if( fscanf( (fd), "%d %d %d %d\n", &(v0), &(v1), &(v2), &(v3) )<4 ) return FALSE;
#define	GET_EDGENO(fd,n)      if( fscanf( (fd), "Edge %d", &(n) )<1 ) return FALSE;
#define	GET_EDGE(fd,v0,v1) if( fscanf( (fd), "%d %d \n", &(v0), &(v1) )<2 ) return FALSE;
